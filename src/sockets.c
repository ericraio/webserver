...
Status client_socket_connect(ClientSocket* s, const char* ip, int port) 
{
  ...
  // Furth, if we get an EINTR followed by EISCONN (interrupt, is-connected),
...
// - Data will be palced in ClientSocket::data,
                                 ...
                                 int timeout_ms)
{
  ...
  while(1) {
    ...
    // If successful, or if we failed with an error but EWOULDBLOCK, return
    if (status.ok || status.errnum != EWOULDBLOCK) { return status; }

    // Check if we timed out
    if (waited > timeout_ms) { return make_status(false, EWOULDBLOCK); }
